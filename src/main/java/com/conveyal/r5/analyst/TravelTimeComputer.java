...
public class TravelTimeComputer {
    ...
    public OneOriginResult computeTravelTimes() throws IOException {
        ...
        if (request.transitModes.isEmpty()) {
            return nonTransitRoute(accessMode, directMode, travelTimeReducer, sr, destinations);
        } else {
            return transitRoute(accessMode, egressMode, travelTimeReducer, sr, destinations);
        }
    }

    /**
     * This search will use no transit. When doing a non-transit walk search, we're
     * not trying to match the behavior of egress and transfer searches which use
     * distance as the quantity to minimize (because they are precalculated and
     * stored as distance, and then converted to times by dividing by speed without
     * regard to weights/penalties for things like stairs). This does mean that
     * walk-only results will not match the walking portion of walk+transit results.
     */
    private OneOriginResult nonTransitRoute(StreetMode accessMode, StreetMode directMode,
            TravelTimeReducer travelTimeReducer, StreetRouter sr, PointSet destinations) {
        // Find travel times for all allowed direct modes and return the shortest time.
        // This stops us from making the (sometimes incorrect) assumption that the
        // dominant street mode will always be the fastest (e.g. walking may be faster than
        // bikeshare for short distances, but bikeshare may be faster for longer distances.)
        List<StreetRouter> directModeRouterOptions = new ArrayList<StreetRouter>();
        for (LegMode directModeOption : request.directModes) {
            if (directModeOption == LegMode.BICYCLE_RENT) {
                StreetRouter bikeShareRouter = new StreetRouter(network.streetLayer);
                bikeShareRouter.profileRequest = request;
                bikeShareRouter.streetMode = accessMode;
                bikeShareRouter.setOrigin(request.fromLat, request.fromLon);
                ...
                bikeShareRouter = bikeQuery.findBikeRentalPath(request, sr, true, false);

                if (bikeShareRouter == null) {
                    ...
                    directModeRouterOptions.add(bikeShareRouter);
                }
            } else {
        ...
        for (int i = 0; i < directModeRouterOptions.size(); i++) {
            ...
            travelTimesByMode[i] = directModeLinkedDestinations.eval(r::getTravelTimeToVertex,
                    offstreetTravelSpeedMillimetersPerSecond).travelTimes;
        }
        int[] travelTimesToTargets = minOf2DArray(travelTimesByMode);

        // Iterate over all destinations ("targets") and at each destination, save the
        // same
     ...
     */
    private OneOriginResult transitRoute(StreetMode accessMode, StreetMode egressMode,
            TravelTimeReducer travelTimeReducer, StreetRouter sr, PointSet destinations) {
        // If the access and egress modes are both the same, the pointset's linkage
        // cache will return two
        ...
        // The code blocks below essentially serve to identify transit stations
        // reachable from the origin and
        // produce a grid of non-transit travel times that will later be merged with the
        // transit travel times.

        // A map from transit stop vertex indices to the travel time it takes to reach
        // those vertices.
        TIntIntMap accessTimes;

        // This will hold the travel times to all destination grid cells reachable
        // without using transit
        // (via only the access/direct mode).
        int[] nonTransitTravelTimesToDestinations;

        // The request has the speed in float meters per second, internally we use
        // integer millimeters per second.
        ...
        if (request.accessModes.contains(LegMode.CAR_PARK)) {
            // Currently first search from origin to P+R is hardcoded as time dominance
            // variable for Max car time seconds
            // Second search from P+R to stops is not actually a search we just return list
            // of all reached stops for each found P+R.
            // If multiple P+Rs reach the same stop, only one with shortest time is
            // returned. Stops were searched for during graph building phase.
            // time to stop is time from CAR streetrouter to stop + CAR PARK time + time to
            // walk to stop based on request walk speed
            // by default 20 CAR PARKS are found it can be changed with sr.maxVertices
            // variable
            ...
            if (sr == null) {
                ...
                // It causes regional analyses to be very slow when there are a large number of
                // disconnected cells.
            ...
            // Special handling for walk search, find distance in seconds and divide to
            // match behavior at egress
            // (in stop trees). For bike/car searches this is immaterial as the access
            // searches are already asymmetric.
            // TODO clarify - I think this is referring to the fact that the egress trees
            // are pre-calculated for a standard speed and must be adjusted.
            ...
            // Convert distances to speeds.
            // getReachedStops returns distances in this case since dominance variable is
            // millimeters,
            ...
            // FIXME is this iterating over every cell in the destination grid just to get
            // the access times around the origin?
            nonTransitTravelTimesToDestinations = accessModeLinkedDestinations.eval(v -> {
                StreetRouter.State state = effectivelyFinalSr.getStateAtVertex(v);
                if (state == null)
                    return FastRaptorWorker.UNREACHED;
                else
                    return state.distance / offstreetTravelSpeedMillimetersPerSecond;
            }, offstreetTravelSpeedMillimetersPerSecond).travelTimes;
        } else {
            // Other modes are already asymmetric with the egress/stop trees, so just do a
            // time-based on street
            ...
            nonTransitTravelTimesToDestinations = accessModeLinkedDestinations.eval(sr::getTravelTimeToVertex,
                    offstreetTravelSpeedMillimetersPerSecond).travelTimes;
        }

        // Short circuit unnecessary transit routing: If the origin was linked to a
        // road, but no transit stations
        ...
        if (accessTimes.isEmpty()) {
            ...
            for (int target = 0; target < nonTransitTravelTimesToDestinations.length; target++) {
                // TODO abstraction for travel time grid, with method to write it directly to
                // reducer
        ...
        if (request.returnPaths || request.travelTimeBreakdown) {
            // By default, this is false and intermediate results (e.g. paths) are
            // discarded.
            // TODO do we really need to save all states just to get the travel time
            // breakdown?
            worker.retainPaths = true;
        }

        // Run the main RAPTOR algorithm to find paths and travel times to all stops in
        // the network.
        // Returns the total travel times as a 2D array of
        // [searchIteration][destinationStopIndex].
        // Additional detailed path information is retained in the FastRaptorWorker
        // after routing.
     ...
     */
    private int[] minOf2DArray(int[][] arr) {
        int[] min = arr[0].clone();
